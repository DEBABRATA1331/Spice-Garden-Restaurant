...
async function main() {
    ...
    // Menu items
    const menuItems = [
        { categoryId: 'cat-starter', name: 'Paneer Tikka', description: 'Marinated cottage cheese grilled in tandoor', price: 280, isVeg: true, isFeatured: true, tags: 'popular,spicy' },
        { categoryId: 'cat-starter', name: 'Chicken 65', description: 'Crispy deep-fried chicken with curry leaves', price: 320, isVeg: false, tags: 'spicy,popular' },
        ...
        { categoryId: 'cat-main', name: 'Biryani - Chicken', description: 'Fragrant basmati rice with spiced chicken', price: 350, isVeg: false, tags: 'popular,bestseller' },
        { categoryId: 'cat-main', name: 'Biryani - Veg', description: 'Aromatic basmati rice with seasonal vegetables', price: 280, isVeg: true },
        ...
        { categoryId: 'cat-dessert', name: 'Gulab Jamun', description: 'Soft milk balls soaked in rose-flavored syrup', price: 120, isVeg: true, tags: 'popular' },
        { categoryId: 'cat-dessert', name: 'Kulfi Falooda', description: 'Traditional Indian ice cream with rose syrup', price: 150, isVeg: true },
    ...
    for (const item of menuItems) {
        const id = `item-${item.name.toLowerCase().replace(/\s/g, '-')}`;
        await prisma.menuItem.upsert({
            where: { id },
            update: {},
            create: { id, restaurantId: restaurant.id, ...item }
        });
    }
